                               ...
                               const char *path)
{
    ...
    if (base->n > 1) {
        for (i = 2; i <= base->n; ++i) {
            ...
            qsort(raw_ngrams, (size_t)base->n_counts[i - 1], sizeof(ngram_raw_t), &ngram_comparator);
                                         ...
                                         logmath_t *lmath)
{
    ...
    if (order > 1) {
        raw_ngrams = ngrams_raw_read_dmp(fp, lmath, counts, order, unigram_next, do_swap);
        ngrams_raw_fix_counts(raw_ngrams, counts, fixed_counts, order);
        for (i = 0; i < order; i++) {
            base->n_counts[i] = fixed_counts[i];
        }
...
static int32 ngram_model_trie_raw_score(ngram_model_t *base, int32 wid, int32 *hist, int32 n_hist, int32 *n_used)
{
    int32 i;
    ngram_model_trie_t *model = (ngram_model_trie_t *)base;
    ...
    return (int32)lm_trie_score(model->trie, model->base.n, wid, hist, n_hist, n_used);
}
